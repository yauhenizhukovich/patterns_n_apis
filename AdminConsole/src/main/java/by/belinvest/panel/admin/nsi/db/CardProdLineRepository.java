...
@Repository
public class CardProdLineRepository {
    private static final String GET_INFO_FROM_DEPFL_PRODUCT_LINE_DIRECTORY_SQL = "select * from card_product_line";
    ...
    private static final String DELETE_INFO_FROM_DEP_PRODUCT_LINE_DIRECTORY_SQL = "DELETE FROM card_product_line where code = ?";
    ...
    @Autowired
    public CardProdLineRepository(@Qualifier("nsiDatasource") DataSource dataSource, CommentRepository commentRepository,
        ...
        simpleJdbcInsert.withTableName("card_product_line").usingGeneratedKeyColumns("code");
    }
    ...
    public boolean create(DepProductLine depProductLine, String nsiId) {
        ...
        jdbcTemplateCatalog.execute("INSERT INTO BP_PRODUCT_LINE  VALUES (" + Integer.parseInt(nsiId) + " ,'CARD_PRODUCT_LINE'," + idD.longValue() + ")");
        return true;
    }

    public boolean update(DepProductLine depProductLine) {
        if (!depProductLine.getIdCard().isEmpty()) {
            jdbcTemplate.update("UPDATE card_product_line SET NAME = '" + depProductLine.getName() + "' WHERE CODE = " + depProductLine.getCode() + "");
            jdbcTemplateCatalog.update("update bp_product_line set bpid=" + Integer.valueOf(depProductLine.getIdCard()) + " where NSI_NAME='CARD_PRODUCT_LINE' and NSI_ID = " + depProductLine.getCode());
        }
    ...
    public boolean remove(String code) {
        ...
        jdbcTemplateCatalog.update("DELETE FROM bp_product_line where NSI_NAME='CARD_PRODUCT_LINE' and NSI_ID = "+code);
        return true;
    }
